...
if ($result = mysqli_query($conn, $sqli)) {
    ...
    data-url2='" . urlencode($secondFilePath) . "'
        ...
        echo "<div class='table-data' style='max-width:50px;'>
        <a id='sendDeleteSvg' href='#' class='sendDeleteSvg' data-id='" . $row["prodOrderNum"] . "'>
...
<script>
    ...
    async function downloadChecked() {
        ...
        const blob = await zip.generateAsync({
            type: 'blob', mimeType: 'application/zip', encoding: 'UTF-8'
        });
        const link = document.createElement('a');
        // link.href = URL.createObjectURL(blob);
        link.href = URL.createObjectURL(new File([blob], 'документы.zip', { type: 'application/zip;charset=utf-8' }));
